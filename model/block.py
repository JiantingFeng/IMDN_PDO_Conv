...
import torch
from PDO_eConv import OpenConv2d, GConv2d


def conv_layer(in_channels, out_channels, kernel_size, stride=1, dilation=1, groups=1, bias=True, is_first=False):
    assert kernel_size in (1, 3)
    padding = int((kernel_size - 1) / 2) * dilation
    if is_first:
        return OpenConv2d(in_channels=in_channels, out_channels=out_channels, stride=stride,
                          dilation=dilation, groups=groups, bias=bias)
    else:
        return GConv2d(in_channels=in_channels, out_channels=out_channels, padding=padding, kernel_size=kernel_size, stride=stride,
                       dilation=dilation, groups=groups, bias=bias)
...
def conv_block(in_nc, out_nc, kernel_size, stride=1, dilation=1, groups=1, bias=True,
               pad_type='zero', norm_type=None, act_type='relu', is_first=False):
    padding = get_valid_padding(kernel_size, dilation)
    p = pad(pad_type, padding) if pad_type and pad_type != 'zero' else None
    padding = padding if pad_type == 'zero' else 0
    if is_first:
        c = OpenConv2d(in_nc, out_nc, stride=stride, padding=padding,
                  dilation=dilation, bias=bias, groups=groups)
    else:
        c = GConv2d(in_nc, out_nc, kernel_size=kernel_size, stride=stride, padding=padding,
                  dilation=dilation, bias=bias, groups=groups)
...
def mean_channels(F):
    assert (F.dim() == 5)
    spatial_sum = F.sum(4, keepdim=True).sum(3, keepdim=True)
    return spatial_sum / (F.size(3) * F.size(4))


def stdv_channels(F):
    assert (F.dim() == 5)
    F_mean = mean_channels(F)
    F_variance = (F - F_mean).pow(2).sum(4, keepdim=True).sum(3, keepdim=True) / (F.size(3) * F.size(4))
...
class CCALayer(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.conv_du = nn.Sequential(
            GConv2d(channel, channel // reduction, stride=1, padding=0, bias=True),
            nn.ReLU(inplace=True),
            GConv2d(channel // reduction, channel, stride=1, padding=0, bias=True),
            nn.Sigmoid()
        )
...
#     def forward(self, input):
#         size = input.size()
#         input.view(size[0], size[1]*size[2], size[3], size[4])
#
#
