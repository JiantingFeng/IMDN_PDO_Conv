...
class IMDN_AS(nn.Module):
    def __init__(self, in_nc=3, nf=64, num_modules=6, out_nc=3, upscale=4):
        ...
        self.fea_conv = nn.Sequential(B.conv_layer(in_nc, nf, kernel_size=3, stride=2, is_first=True),
                                      nn.LeakyReLU(0.05),
...
class IMDN(nn.Module):
    def __init__(self, in_nc=3, nf=64, num_modules=6, out_nc=3, upscale=4):
        ...
        self.fea_conv = B.conv_layer(in_nc, nf, kernel_size=3, is_first=True)
...
class IMDN_RTC(nn.Module):
    def __init__(self, in_nc=3, nf=12, num_modules=5, out_nc=3, upscale=2):
        ...
        fea_conv = [B.conv_layer(in_nc, nf, kernel_size=3, is_first=True)]
...
class IMDN_RTE(nn.Module):
    def __init__(self, upscale=2, in_nc=3, nf=20, out_nc=3):
        ...
        self.fea_conv = nn.Sequential(B.conv_layer(in_nc, nf, 3, is_first=True),
    ...
    def forward(self, input):
        ...
        size = out_lr.size()
        out_lr = torch.sum(out_lr, dim=2, keepdim=False, dtype=None)    #将8个通道求和
        output = self.upsampler(out_lr)
        return output
